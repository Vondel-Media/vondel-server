...
BUILDINFO_PKG := github.com/Vondel-Media/vondel-server/internal/buildinfo
...
build: frontend
	go build -ldflags "$(GO_LDFLAGS)" -o vondel ./cmd/vondel/
...
dev-proxy:
	go run ./cmd/vondel/ --mode=proxy

# Run a transcode node (HLS transcode worker, no DB required)
dev-transcode:
	go run ./cmd/vondel/ --mode=transcode
...
migrate-status:
	go run ./cmd/vondel/ --env "$(ENV_FILE)" --migrate-status
...
migrate-down-to:
	...
	go run ./cmd/vondel/ --env "$(ENV_FILE)" --migrate-down-to "$(VERSION)"

# Apply pending Goose migrations through Silo's bootstrapping runner.
migrate-up:
	go run ./cmd/vondel/ --env "$(ENV_FILE)" --migrate-only
...
jellyfin-web:
	go run ./cmd/vondel/ compat-web install --dir "$(JELLYFIN_WEB_INSTALL_DIR)" --version "$(JELLYFIN_WEB_VERSION)"
