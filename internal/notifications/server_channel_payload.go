...
// vondelSenderName labels Discord posts and embed footers.
const vondelSenderName = "Vondel"
...
func BuildServerChannelDiscordContent(groups []ContentGroup, test bool) ([]byte, error) {
	...
	body := discordWebhookBody{Embeds: embeds, Username: vondelSenderName}
	if overflow > 0 {
...
func BuildServerChannelRequestDiscord(event string, info RequestEventInfo) ([]byte, error) {
	...
	embed := discordEmbed{
		...
		Footer:      &discordEmbedFooter{Text: vondelSenderName},
		Timestamp:   time.Now().UTC().Format(time.RFC3339),
		Fields:      fields,
	}
	...
	enforceDiscordTotalLimit(&embed)
	body := discordWebhookBody{Embeds: []discordEmbed{embed}, Username: vondelSenderName}
