...
func TestRenderIndexHTMLReplacesTitle(t *testing.T) {
	in := []byte(`<html><head><title>Vondel</title></head><body></body></html>`)
...
func TestRenderIndexHTMLAgainstRealShell(t *testing.T) {
	...
	if !strings.Contains(string(data), "<title>Vondel</title>") {
		t.Fatalf("web/index.html no longer contains the expected <title>Vondel</title>; update RenderIndexHTML")
	}
