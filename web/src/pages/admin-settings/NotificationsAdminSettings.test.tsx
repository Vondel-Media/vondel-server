...
describe("NotificationsAdminSettings", () => {
  it("registers Vondel Push Relay settings with the shared settings form", () => {
  ...
  it("shows the Vondel Push Relay channel status", async () => {
    useSettingsFormMock.mockReturnValue(makeForm());

    render(renderPage());

    expect(screen.getByText("Vondel Push Relay")).toBeInTheDocument();
    ...
    expect(screen.getByText(/content-free request to Vondel's push relay/)).toBeInTheDocument();
