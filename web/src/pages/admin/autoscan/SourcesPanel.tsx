          ...
          {preview && (
            ...
            >
              ...
              {preview.unmatched.length > 0 && (
                <CollapsibleList
                  title={`No Vondel match (${preview.unmatched.length})`}
...
interface AddSourceForm {
  ...
  /** False while a required or validated config field is unsatisfied. */
  configValid: boolean;
  /** Webhook-only: arr path -> Vondel path rows, seeded from library paths. */
...
const DELIVERY_MODE_COPY: Record<AutoscanDeliveryMode, { title: string; description: string }> = {
  webhook: {
    title: "The service tells Vondel",
    description:
      "Instant. Paste one URL into the service's webhook settings. No credentials stored here.",
  },
  poll: {
    title: "Vondel checks the service",
    description: "Vondel asks on a schedule. Works without changing anything upstream.",
  },
};
...
function AddSourceDialog({
  ...
  // Poll interval only means something when Vondel is the one asking.
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            {createdWebhookSource
              ...
              : "Pick what you want Vondel to watch. Each source only asks for what it actually needs."}
          </DialogDescription>
        </DialogHeader>
        ...
        {createdWebhookSource ? (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>What should Vondel watch?</Label>
            ...
            {showDeliveryChoice && (
              <div className="space-y-2">
                <Label>How should Vondel hear about changes?</Label>
                <div className="grid gap-2 sm:grid-cols-2">
