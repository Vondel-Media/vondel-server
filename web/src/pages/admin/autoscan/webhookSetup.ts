...
/**
 * The arr notification triggers Vondel actually consumes.
 *
...
export interface WebhookTrigger {
  ...
  label: string;
  /** Why Vondel wants it, shown as helper text. */
  reason: string;
...
const SHARED_TRIGGERS: WebhookTrigger[] = [
  ...
  {
    label: "On Rename",
    reason: "Fires when files are renamed, so Vondel does not lose track of them.",
    required: false,
  },
];
...
const SONARR_TRIGGERS: WebhookTrigger[] = [
  ...SHARED_TRIGGERS,
  {
    ...
    reason: "Lets Vondel drop episodes you removed, instead of leaving dead entries.",
    required: false,
  },
];
...
const RADARR_TRIGGERS: WebhookTrigger[] = [
  ...SHARED_TRIGGERS,
  {
    ...
    reason: "Lets Vondel drop movies you removed, instead of leaving dead entries.",
    required: false,
  },
];
...
export function triggersFor(provider: AutoscanWebhookProvider | "auto"): WebhookTrigger[] {
  ...
  return [
    ...SHARED_TRIGGERS,
    {
      ...
      reason: "Whichever your service offers — lets Vondel drop files you removed.",
      required: false,
    },
  ];
}
 ...
 * (`from`) and the Vondel path it corresponds to (`to`).
 *
 ...
 * but only when the arr-side tree mirrors the Vondel-side tree below that point.
 *
 ...
 * collapsed row instead of asking them to retype the Vondel side.
 *
 ...
 * starts with a real Vondel path on the `to` side and a blank `from` for the
