...
Vondel is a self-hosted media streaming server for your movies, shows, music, and books. Point it at your media folders and stream to your devices — at home or away — with direct play, remuxing, and hardware-accelerated transcoding handled automatically.

Vondel is an independent hard fork of [Silo Server](https://github.com/Silo-Server/silo-server), starting from upstream revision `1dcdd4b2`. It is not affiliated with or endorsed by Silo Media L.L.C. Existing protocol identifiers are retained where needed so Vondel remains compatible with official Silo servers and clients.
...
The easiest way to run Vondel is with Docker Compose 2.24 or newer. The default stack assumes you do
...
1. **Create a `.env` file**
   ...
   This replaces the development database password from `.env.example` and creates the key Vondel
...
2. **Set your media path**
   ...
   `MEDIA_ROOT` is the one value most users need to change. You can also override `VONDEL_DATA_ROOT` if you do not want bind mounts under `/opt/vondel`, and change ports if the defaults conflict with something else on the host.
...
3. **Start the default integrated stack**
   ...
   This starts PostgreSQL, Redis, and the integrated Vondel server. The app is available at `http://localhost:8090`. Jellyfin-compatible app support is disabled until an administrator enables it in onboarding or admin settings.

   If you already have PostgreSQL and Redis available, omit those bundled service examples from compose and point Vondel at your existing `DATABASE_URL` and `REDIS_URL` instead.
...
By default, data is stored under `/opt/vondel`:

- `/opt/vondel/postgres`
- `/opt/vondel/redis`
- `/opt/vondel/plugins`
- `/opt/vondel/compat`
- `/opt/vondel/transcode`
- `/opt/vondel/catalog-seeds`

The optional `search` profile also stores its index under `/opt/vondel/meilisearch`.
...
`openssl rand -hex 32`. After Vondel starts, choose Meilisearch under **Admin > Settings > Search**,
set the URL to `http://meilisearch:7700`, enter the same key as the API key, test the connection,
and save. Restart Vondel, then rebuild the catalog search index from the same page. Vondel continues
...
Existing Silo operators should read the [Silo-to-Vondel migration notes](docs/silo-to-vondel-migration.md) before changing images or host paths.

## Configuration

Vondel requires `DATABASE_URL` and `SECRET_KEY` when running from source or against external
...
It enables Vondel's [pgtune](https://github.com/le0pard/pgtune)-style OLTP tuning
by default:
...
When enabled, Vondel connects with `DATABASE_URL` and applies recommendations with
...
too, and Vondel logs the setting names so you can restart PostgreSQL once:
...
For `POSTGRES_TUNE_MEMORY=auto`, Vondel uses the first trustworthy memory source:
...
detected RAM so Vondel, Redis, plugins, transcodes, and the OS retain headroom.
...
| `POSTGRES_TUNE_CONNECTIONS` | `100` | PostgreSQL `max_connections`; automatically raised if Vondel's app pool is configured higher. |
...
override these env vars. Set `POSTGRES_TUNE=off` when you do not want Vondel to
...
If you prefer running Vondel without Docker:
...
2. **Configure the source process**
   ...
   printf '\nSECRET_KEY=%s\nDATABASE_URL=%s\nREDIS_URL=%s\n' \
     ...
     'postgres://vondel:vondel@localhost:5432/vondel?sslmode=disable' \
...
Vondel is open source and contributions are welcome. See [DEVELOPMENT.md](DEVELOPMENT.md) for building from source in a dev workflow, running tests, database migrations, and project layout, and [CONTRIBUTING.md](CONTRIBUTING.md) for contribution expectations, merge request guidance, and the policy for AI-assisted submissions.

## License & Trademarks

Vondel's source code is licensed under the **GNU Affero General Public License
v3.0 or later** (`AGPL-3.0-or-later`) — see [LICENSE](LICENSE).

Vondel retains the upstream copyright notices required by the AGPL. The Vondel
name and original visual assets are separate from the source-code license grant;
